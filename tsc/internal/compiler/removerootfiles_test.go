...
func TestRemoveRootFilesMatchesRebuild(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			// Falls back to a full rebuild here: upstream unified the rootDir diagnostics
			// this optimization once tracked separately into the single processing-diagnostic
			// list, so a removal whose base carried any processing diagnostic is rebuilt
			// rather than derived. The rebuild is correct; only the incremental fast path is
