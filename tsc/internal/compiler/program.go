...
func (p *Program) reuseCommonSourceDirectory(base *Program, addedFiles []*ast.SourceFile, removed *removedRoots) bool {
	// The derived program computes its common source directory lazily and its rootDir
	// diagnostics through verifyCompilerOptions, so there is nothing to reuse here under
	// upstream's unified-diagnostics / sync.Once model. Kept as a no-op so UpdateRootFiles
	// stays correct; the micro-optimization of reusing the base's computation is dropped.
	_ = addedFiles
	_ = removed
	return false
}
